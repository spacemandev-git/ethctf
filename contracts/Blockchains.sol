...
contract Blockchains{
  ...
  // Get Chain Stats
  function getChainInfo(chainID) public view chainExists(chainID)
  returns( address chainOwner, type chainType){
    returns (blockchainsById[chainID].chainOwner,blockchainsById[chainID].chainType);
  }

  function getChainNodes(chainID) public view chainExists(chainID){
    return blockchainsById[chainID].nodes;
  }

  function getChainAttributes(chainID) public view chainExists(chainID) 
  ...
  function getChainBlockCreationInfo(chainID) public view chainExists(chainID)
  ...
  function getPlayerRewards(uint256 chainID, address hacker) public view chainExist(chainID) returns(uint256 _playerRewards){
  ...
  function cardsContributedByPlayer(uint256 chainID, address hacker) public view returns(uint256 cardsContributed){
    cardsContributed = blockchainsById[chainID].playerContributions[hacsker];
  }

  function isBlockchainAlive(chainID) public view chainExist(chainID)returns(bool){
  ...
  function mutateChainBlockHeight(uint256 chainID, uint256 address newHeight) public isMutator() chainExists(chainID) {
  ...
  function mutateChainLastBlockNum(uint256 chainID, address newNum) public isMutator() chainExists(chainID) {
